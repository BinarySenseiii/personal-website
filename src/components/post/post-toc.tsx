...
const PostTableOfContent = ({toc}: {toc: Post['toc']}) => {
  ...
  return (
    <Accordion
      ...
      collapsible
      className="w-full"
      onValueChange={setTocValue}
    >
      <AccordionItem value="post-toc">
        <AccordionTrigger className="font-semibold text-sm font-ubuntu bg-neutral-100 dark:bg-neutral-800 el-focus-styles rounded-sm p-2">
          <p>
            Table of Contents{' '}
            <span className="text-muted-foreground">
              (Click to {tocValue ? 'Close' : 'Open'})
            </span>
          </p>
        </AccordionTrigger>
